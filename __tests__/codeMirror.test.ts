...
describe('codemirror', () => {
  ...
  it('should work when passed a non-string value', () => {
    expect(() => syntaxHighlighter(false, 'text')).not.toThrow();
  });

  it('should sanitize plain text language', () => {
    render(syntaxHighlighter('& < > " \' /', 'text'));
    expect(screen.getByText('& < > " \' /')).toBeVisible();
  });

  it('should sanitize mode', () => {
